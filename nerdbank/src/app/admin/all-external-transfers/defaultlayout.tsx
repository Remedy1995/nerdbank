...
import { fetchGetDeposits } from "../../../../bankapi/bankapiComponents";
...
export default function AllExternalTransfers() {
  ...
  const { data, isLoading } = useQuery<BulkTransferData>({
    ...
    queryFn: async () => {
      const result = await fetchGetDeposits({
  ...
  const selectedData = !Array.isArray(data)
    ? []
    : data && data.filter((sel) => sel?._id === id);
  const pagesToShow = 5;
                          ...
                          return (
                            ...
                            >
                              <div className="row align-items-center flex-row overflow-auto">
                                ...
                                <div className="col-4 col-sm-5 overflow-auto text-center">
                                  {" "}
