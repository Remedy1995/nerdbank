...
export default function TransferMoney() {
     ...
     console.log('user Info',userInfo?.message?.firstname + " "+ userInfo?.message?.lastname)




   React.useEffect(()=> {
         const senderAccountInfo : any= userAccount?.data?.accountNumber + "-" +userInfo?.message?.firstname + " "+ userInfo?.message?.lastname;
  ...
  const onSubmit = (data: string | any) => {
    ...
    if (watchTransferTypeSelected === 'self') {
      transferMoneyData  = transferMoneyData.map((val: { body: { amount: string, descriptionOfTransfer: string } }) => {
    ...
    console.log(data)
    navigation.push("/client/verify-pin")
  }
  return (<>
     ...
     {/* <div className="bg-primary">
      ...
      <div id="content" className="py-4">
        <div className="container">
          ...
          <p className="lead text-center mb-4">Transfer Money at anytime, anywhere in the world.</p>
          <div className="row">
            <div className="col-md-9 col-lg-7 col-xl-6 mx-auto">
              <div className="bg-white shadow-sm rounded p-3 pt-sm-4 pb-sm-5 px-sm-5 mb-4">
                <h3 className="text-5 fw-400 mb-3 mb-sm-4">Receipient Details</h3>
                <hr className="mx-n3 mx-sm-n5 mb-4" />
                <form id="form-send-money" onSubmit={handleSubmit(onSubmit)}>
                  <div className="mb-3">
                    <label htmlFor="payerName" className="form-label">Transfer From Account</label>
                    {  <input type="text"
                      className="form-control" placeholder="Account Information" value={userAccount && userInfo ? AccountInfo : "loading..."}
                      {...register("senderAccountInfo", { required: true, disabled: true })}
                      aria-invalid={errors.senderAccountInfo ? "true" : "false"} />}
                    {errors.senderAccountInfo?.type === "required" && (
                      <p className="text-danger" role="alert">Account Name is required</p>
                    )}
                  </div>
                  <div className="mb-3">
                    <label htmlFor="transferType" className="form-label">Type Of Transfer</label>
                    <select
                    ...
                    {errors.transferType?.type === "required" && (
                      <p className="text-danger" role="alert">Transfer Type is required</p>
                    )}
                  </div>
                  {watchTransferTypeSelected === 'external' && <div className="mb-3">
                    <label htmlFor="payerName" className="form-label">Account Name</label>
                    <input type="text"
                      className="form-control" placeholder="Enter Account Name"
                  ...
                  {['self', 'external'].includes(watchTransferTypeSelected) && <div className="mb-3">
                    <label htmlFor="payerName" className="form-label">Amount</label>
                    <input type="text"
                      className="form-control" placeholder="Enter Amount"
                      {...register("amount", { required: true })}
                      aria-invalid={errors.amount ? "true" : "false"} />
                    {errors.amount?.type === "required" && (
                      <p className="text-danger" role="alert">Amount is required</p>
                    )}
                  </div>}
                  {['self', 'external'].includes(watchTransferTypeSelected) && <div className="mb-3">
                    <label htmlFor="payerName" className="form-label">Account Number</label>
                    <input type="text"
                    ...
                    {errors.accountNumber?.type === "required" && (
                      <p className="text-danger" role="alert">Account Number is required</p>
                    )}
                  </div>}
                  {watchTransferTypeSelected === 'external' && <div className="mb-3">
                    <label htmlFor="payerName" className="form-label">Account Bank Name</label>
                    <input type="text"
                      className="form-control" placeholder="Enter Account Bank Name"
                      {...register("accountBankName", { required: true })}
                      aria-invalid={errors.accountBankName ? "true" : "false"} />
                    {errors.accountBankName?.type === "required" && (
                      <p className="text-danger" role="alert">Account Bank Name is required</p>
                    )}
                  </div>}
                  ...
                  {['self', 'external'].includes(watchTransferTypeSelected) && <div className="mb-3">
                    ...
                    {errors.accountType?.type === "required" && (
                      <p className="text-danger" role="alert">Account Type is required</p>
                    )}
                  </div>}
                  {watchTransferTypeSelected === 'external' && <div className="mb-3">
                    <label htmlFor="payerName" className="form-label"> Routing Transit Number</label>
                    <input type="text"
                      className="form-control" placeholder="Enter Routing Transit Number"
                      {...register("accountBankRoutingTransitNumber", { required: true })}
                      aria-invalid={errors.accountBankRoutingTransitNumber ? "true" : "false"} />
                    {errors.accountBankRoutingTransitNumber?.type === "required" && (
                      <p className="text-danger" role="alert">Routing Transit Number is required</p>
                    )}
                  </div>}
                  {watchTransferTypeSelected === 'external' && <div className="mb-3">
                    <label htmlFor="payerName" className="form-label">Bank Swift Code</label>
                    <input type="text"
                      className="form-control" placeholder="Enter Bank Swift Code"
                      {...register("accountBankSwiftCode", { required: true })}
                      aria-invalid={errors.accountBankSwiftCode ? "true" : "false"} />
                    {errors.accountBankSwiftCode?.type === "required" && (
                      <p className="text-danger" role="alert">Bank Swift Code is required</p>
                    )}
                  </div>}
                  {['self', 'external'].includes(watchTransferTypeSelected) && <div className="mb-3">
                    <label htmlFor="description" className="form-label">Enter Description of Transfer</label>
                    <textarea className="form-control"
                      {...register("descriptionOfTransfer", { required: true })}
                      aria-invalid={errors.descriptionOfTransfer ? "true" : "false"}
                      rows={4} id="description" placeholder="Description of Transfer"></textarea>
                    {errors.descriptionOfTransfer?.type === "required" && (
                      <p className="text-danger" role="alert">Description Of Transfer is required</p>
                    )}
                  </div>}
                  {['self', 'external'].includes(watchTransferTypeSelected) && <div className="d-grid mt-4">
