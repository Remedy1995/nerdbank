...
export default function ConfirmTransfer() {
    ...
    onSuccess: (data) => {
      ...
      if (data) {
        ...
        if (dataMessage.includes("message")) {
          navigation.push("/client/success-transfer");
        }
      }
    },
    onError: (error) => {
      ...
      if (error) {
        ...
        setShowErrorMessage(
          errorMessage[2]["message"] || errorMessage[2]["error"]
        );
        setVisible(true);
      }
    },
  ...
  const spliceAccountNumber =
    transferType !== "self"
      ? receiverAccount?.accountNumber.slice(-5)
      : receiverAccountData?.toString()?.slice(-5);
  const padAccountNumber =
    transferType !== "self"
      ? spliceAccountNumber?.padStart(
          Number(receiverAccount?.accountNumber.length),
          "X"
        ) || ""
      : spliceAccountNumber?.padStart(
          Number(receiverAccountData?.toString()?.length),
          "X"
        );
  ...
  const transferMoney = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (transferType === "self") {
      const removeExtra = transferMoneyData
        .map((val) => {
          if (val) {
            const { body } = val;
              ...
              <div className="col-md-9 col-lg-7 col-xl-6 mx-auto">
                ...
                <div className="bg-white shadow-sm rounded p-3 pt-sm-5 pb-sm-5 px-sm-5 mb-4">
                  <p className="lead text-center alert alert-info">
                    You are Transfering money
                    <br />
                    to
                    <br />
                    <span className="fw-500">
                      {transferType !== "self"
                        ? receiverAccount?.accountBankName
                        : null}{" "}
                      {transferType !== "self" && "-"}
                      {padAccountNumber}
                    </span>
                  ...
                  <p className="mb-2 mt-4">
                    Amount to Transfer{" "}
                    <span className="text-3 float-end">
                      {" "}
                      {Number(amount)} £
                    </span>
                  </p>
                  ...
                  <p className="text-4 fw-500">
                    Total<span className="float-end">{Number(amount)} £</span>
                  </p>
